...
public class ExtAudioRecorder {
	...
	public static String spxPath  = "/sdcard/cn.com.easytaxi/voice.spx";
	...
	private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener() {
		public void onPeriodicNotification(AudioRecord recorder) {
			...
			// buffer
			int realSize = audioRecorder.read(shortBuffer, 0, shortBuffer.length); // Fill
																			// buffer
			try {
				...
				int n = 0;
				for(int j = 0 ;j < realSize;j =j+2){
					if(j/2 == 0){
						continue;
					}
					realData[n] =  shortBuffer[j];
					n++;
				}
				
//				System.arraycopy(shortBuffer, 0, realData, 0, realSize);
				processedData = speexEncoder.encode(realData);
				Log.d("-----------", "realSize" + realSize );
...
//		String spxFilePath = ETApp.getInstance().soundPath;
		spxPath = ETApp.getInstance().getMobileInfo().getSDCardPath()+"voiceyd.spx";
		//int pcmPacketSize = 2 * 1 * frameSize; // 320
		try {
			writer.open(spxPath);
			writer.writeHeader("Encoded with: easytaxi");
		...
		try {
			...
			if (rUncompressed) { // RECORDING_UNCOMPRESSED
				...
				audioRecorder = new AudioRecord(audioSource, sRate, channelConfig, audioFormat, bufferSize);
			...
			if (state == State.INITIALIZING) {
				if (rUncompressed) {
					if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED) & (filePath != null)) {
						...
						buffer = new byte[bufferSize]; // 分配内存块大小 frame * 采样路/
														// 8bit * 通道个数 = 字节数
						// buffer = new byte[framePeriod*bSamples/8*nChannels];
						// //分配内存块大小 frame * 采样路/ 8bit * 通道个数 = 字节数
